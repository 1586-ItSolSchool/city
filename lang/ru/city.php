<?php
/**
 * changelog: 2019010601 initial release
 * @author ddsh
 * @see https://docs.moodle.org/dev/Activity_modules#Lang_Folder
 */
$string['modulename'] = 'Цифровой Город';
$string['modulenameplural'] = '';

// строки для меню добавления активности. при любом изменении строк нужно очищать кеши moodle.
$string['myprofile'] = 'Мой игровой профиль';
$string['myprofilehelp'] = 'Эта страница позволяет игрокам просматривать их кошельки и их контракты';
$string['overview'] = 'Обзор экономики';
$string['overviewhelp'] = 'Эта страница позволяет игрокам просматривать информацию обо всей экономике Города';

$string['city:viewalltransactions'] = 'city:viewalltransactions';
$string['city:editalltransactions'] = 'city:editalltransactions';
$string['city:viewallcontracts'] = 'city:viewallcontracts';
$string['city:editallcontracts'] = 'city:editallcontracts';
$string['city:commentasminecon'] = 'city:commentasminecon';
$string['city:addinstance'] = 'city:addinstance';
?>